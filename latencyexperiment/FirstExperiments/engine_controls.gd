...
func _on_phys_h_slider_value_changed(value: float) -> void:
	$PhysicsTicks/Value.text = str(value)
...
func random_walk_sliders():
	...
	time_scale_h_slider.value += randf_range(0,2)
